# Pruebas BDD del Sistema de Gestión de Residuos Latacunga
...
Write-Host "`n🔍 Escenario 1: Disponibilidad del Sistema" -ForegroundColor Cyan
...
Write-Host "`n👤 Escenario 2: Registro y Autenticación" -ForegroundColor Cyan
...
if ($loginResponse -and $loginResponse.Content) {
    try {
        ...
        Write-Host "⚠️ No se pudo extraer el token de acceso" -ForegroundColor Yellow
    }
}

# Escenario 4: Acceso a endpoint protegido
Write-Host "`n🔐 Escenario 4: Endpoints Protegidos" -ForegroundColor Cyan
...
Write-Host "`n⚙️ Escenario 6: Funcionalidades del Sistema" -ForegroundColor Cyan

Test-Endpoint "API Schema" "$baseUrl/schema/" "GET"
Test-Endpoint "RabbitMQ Management" "http://localhost:15672/api/alarms" "GET" @{} @{} 401

# Generar reporte
Write-Host "`n📋 REPORTE DE PRUEBAS" -ForegroundColor Green
Write-Host "=" * 50
...
if ($totalTests -gt 0) {
    ...
    Write-Host "🎯 Porcentaje de éxito: $successRate%" -ForegroundColor White
}
...
Write-Host "`n💾 Reporte guardado en: test_report.json" -ForegroundColor Green

# Verificar estado de contenedores
Write-Host "`n🐳 Estado de Contenedores Docker:" -ForegroundColor Cyan
try {
...
Write-Host "Sistema de Gestión de Residuos Latacunga funcionando correctamente!" -ForegroundColor Green